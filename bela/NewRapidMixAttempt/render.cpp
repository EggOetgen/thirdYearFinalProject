...
vector<double> major = { 261.63,   0.1, 0.9, 0,  50, 500, 50, 500, 
						 349.23,   0.1, 0.9, 0,  50, 500, 50, 500,
				     	  	392,   0.1, 0.9, 0,  50,  500, 50, 500,
						523.25,    0.1, 0.9, 0,   50,  500, 50, 500, 
				  
};

// vector<double> major = { 32.7,   0.1, 0.9, 0,  50, 500, 50, 500, 
// 						  174.61,   0.1, 0.9, 0,  50, 500, 50, 500,
// 				     	  	 98.6,   0.1, 0.9, 0,  50,  500, 50, 500,
// 						130.81,    0.1, 0.9, 0,   50,  500, 50, 500, 
				  
// };
// vector<double> drumKitLong = { 4.184,  0.692,      30,  200.,
// 						      5,         0.5,       0.1,  100., 
// 					 	      2,        0.2,  504., 0.,  84.5,
// 						   4.184,    0.692,     0.,  84.5, 
				  
// };
// // vector<double> drumKit = {   32.7,  4.184, 1.,    0.2,  0.0,   0.002, 0.1, 2., 100., 0.,  84., 0.,   228.,
// // 						   174.61,    3.6, 1.,   0.14, 0.14,    0.85, 0.1, 1., 428., 0., 100., 1.,    28.,
// // 					 	     98.6,      0, 1.,     1.,  0.5,    0.01, 0.0, 1., 504., 0.,  84., 1.,    50.,
// // 						   130.81,  4.184, 1.,    0.2,  0.0,     1.0, 0.1, 1., 504., 0.,  84., 0.,   228. 
				  
// // };
// 	//freq, pma, gain_, om, tm, nm, oa, oh, oR, pa, pr, na, nr
// vector<double> major = {   0.1, 1.,  50, 500,
// 						   0.1,  0,  50, 500,
// 				     	   0.1,  0, 50, 500,
// 						   0.1,  0,  50, 500
...
//	yn = w × xn + (1 – w) × yn – 1
	...
	//	buttonPressOneToFour = (filterWeight * buttonPressOneToFour) + ((1.-filterWeight) * prev1t4);
	//	prev1t4 = buttonPressOneToFour;
...
//	buttonInputs[2]=digitalRead(cmontext, 0, P8_09); //read the value of the button
...
//	buttonInputs[5]=digitalRead(context, 0, P8_12); //read the value of the button
	...
	// 		//	Bela_scheduleAuxiliaryTask(gGenerateEuclideanSequence);
			...
			float pmRls = ::map(pot3,0,0.82, 10, 1000);
			
		//	cout << om << " " << pma << " " << pmAtk << " "<< pmRls <<endl;
	...
	for(uint8_t i = 0 ; i < 8; i++){
			...
			testBrain.voices[testBrain.selectedVoice].oscAmpEnv.setAttack( pmAtk);
			testBrain.voices[testBrain.selectedVoice].oscAmpEnv.setRelease( pmRls);
			testBrain.voices[testBrain.selectedVoice].pmAtk = pmAtk;
			testBrain.voices[testBrain.selectedVoice].pmRls = pmRls;
			testBrain.voices[testBrain.selectedVoice].oscAtk = pmAtk;
			testBrain.voices[testBrain.selectedVoice].oscRls = pmRls;
		//	testBrain.voices[testBrain.selectedVoice].oscAmpEnv.setRelease( pmRls);
				//freq, pma, gain_, om, tm, nm, oa, oh, oR, pa, pr, na, nr
